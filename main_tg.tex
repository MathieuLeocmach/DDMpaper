...
\begin{figure}
\includegraphics[width=\linewidth]{Lamelle+Photos}
...
\begin{figure*}
\includegraphics[width=\linewidth]{Diff_TF}
...
Following \citet{1_BactMobil} we chose a Schulz distribution for $P(v)$ which respects the apparent dynamic of bacteria, i.e. peaked around the average velocity and going to 0 when $v \rightarrow \infty$:
\begin{multline}
...
We fitted $\mathcal{D} (q, \Delta t)$ with the following initial parameters:
\begin{equation}
\left\{
\begin{array}{rcl}
A_0 &=& \mathcal{D} (q,\Delta t_\text{max}) - \mathcal{D} (q,\Delta t_\text{min}) \\
...
The two diffusion phenomena can be observed on the plot of $f(q, \Delta t)$ (FIG. \ref{ISFBacteria}a) as two separate steps in the decorrelation. The first step for $\Delta t < \SI{0.3}{\second}$, is due to the running motion of bacteria, Brownian motion is negligible. It is confirmed by the collapse of the first step of each curve when plotted as a function of $\Delta t q$ (dimension of an inverse velocity), see FIG. \ref{ISFBacteria}b. The second step is due to Brownian diffusion, confirmed by the collapse of the second step of each curve when plotted as a function of $\Delta t q^2$ (dimension of an inverse diffusion coefficient), see FIG. \ref{ISFBacteria}c.
...
Fitting parameters are displayed in FIG. \ref{FitParametersBacteria} function of $q$. We observe the same three regimes of $q$ as for the colloids: insufficient statistics in regime (1), small signal to noise ratio in regime (3) and reasonable fit in regime (2). In particular, the fraction $\alpha$ of motile bacteria, supposed to be independent of $q$, is increasing and in regime (1) and decreasing in regime (3) while almost constant in regime (2) with an average value of $\overline{\alpha} = 65\%$. However, the parameter $Z(q)$ that quantifies the spread of the velocity distribution is not constant in regime (2) XXX que dire là dessus ?XXX

Within regime (2) the run time and the diffusion time can be reasonably fitted in logarithmic scale by strait lines of respective slopes -1 (ballistic) and -2 (diffusion) whose respective intersects yield $\overline{v} = \SI{21.6}{\micro\meter\per\second}$ and $D = \SI{0.17}{\micro\meter\squared\per\second}$. The \textsc{Stokes-Einstein} relation considering spherical bacteria of diameter \SI{3}{\micro\meter}, a viscosity $\eta = \SI{1}{\milli\pascal\second}$ and a temperature $T=\SI{293}{\kelvin}$, yields $D_\text{SE} = \SI{0.14}{\micro\meter\squared\per\second}$, a very good agreement even though we are not considering the real shape of bacteria.


%The average value of $Z(q)$ given by the fit allows to plot the velocity distribution of bacteria supposing that the Schulz distribution is a good model for the actual bacteria dynamics (FIG. \ref{ISFBacteria}.b).



In our model we are not characterizing the motion of bacteria over many tumbles. As it is also a diffusive phenomenon, it would be interesting to compare it to the Brownian motion to see at which time they are dominant. A theoretical model has been proposed by K. Martens et al. \cite{6_martens2012probability} to take account of tumble in the model of the ISF.
...
\begin{figure}
	...
	\caption{Fraction of motile bacteria, characteristic running and diffusive time, and fit parameters A(q), B(q).}
...
Differential Dynamic Microscopy (DDM) is a microscopy technique that probes the dynamics of a system of particles using a microscope, a camera and numerical computations. We exposed this technique in the well-known case of simple Brownian motion before applying it to the more complex case of motile bacteria. We have shown how to extract physically relevant information from DDM analysis: diffusion coefficient, velocity, scaling of ballistic and diffusive motion.

Since DDM is based on microscopy, it can benefit from techniques more advanced than the bright field imaging we used here. It is possible to use fluorescence microscopy (one or two photon) to tell apart colloidal probes in a crowed medium, for example nanoparticles swallowed by a living cell. Confocal microscopy, polarized light microscopy, dark field microscopy or any super-resolution microscopy technique can be used to produce clear images of specific systems, images that can be processed using DDM. Even macroscopic systems, like human crowds shot from above, can be studied by DDM. In that sense, DDM opens much more possibilities than dynamic light scattering and can be used in various contexts both in the lab and in the classroom.

Compared to particle tracking, DDM is less limited by the diffraction limit and is able to yield information on scales much larger than the size of the particles. When the particles are too fast for trajectory reconstruction, DDM can still gather correlation on larger length scales~\cite{Hendricks2015} and characterize the dynamics of the system. Also  contrary to tracking algorithms, DDM algorithm is straightforward to implement, at the programming level of most undergraduate students. Since FFT is explicitly performed, a lab work on DDM is a great opportunity to become familiar with reciprocal units, the range of accessible wavenumbers and other nitty-gritty details necessary to tame the power of the Fourier transform.
