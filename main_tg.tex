...
&=& \SI{4.0e-21}{\joule} \\ \nonumber
E_\text{g} &=& \frac{4}{3} \pi r^3 (\rho_\text{coll} - \rho_\text{solution}) g  2r \nonumber \\ \nonumber
&=& \SI{3.1e-22}{\joule} \\ \nonumber
...
\begin{tabular}{|m{0.7\linewidth}|m{0.28\linewidth}|}
  \hline
  \vspace{1mm}
  Molecular diameter of water (H$_2$O) \citep{17_marcus1998properties}  & \vspace{2mm} $\SI{0.34}{\nano\meter}$ \vspace{1mm}  \\
  \hline
  \vspace{1mm}
  Temperature $T$ of the suspension & \vspace{2mm} $\SI{20}{\degree C}$ \vspace{1mm} \\
  \hline
  \vspace{1mm}
 Tabulated viscosity of water at $T$ & \vspace{2mm} $\SI{1.002}{\milli\pascal\second}$ \vspace{1mm} \\
  \hline
  \vspace{1mm}
  Density $\rho_\text{coll}$ of polystyrene & \vspace{2mm} $\SI{1.04}{\gram\per\centi\meter\cubed}$ \vspace{1mm} \\
  \hline
  \vspace{1mm}
 Colloid diameter $2r$ & \vspace{2mm} $\SI{1}{\micro\meter}$ \vspace{1mm} \\
  \hline
  \end{tabular}
